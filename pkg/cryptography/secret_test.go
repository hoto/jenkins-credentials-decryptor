...
func Test_decrypts_secret(t *testing.T) {
	...
	expectedDecryptedSecret, _ := ioutil.ReadFile("../../test/resources/decrypted/hudson.util.Secret")

	actualDecryptedSecret, _ := DecryptHudsonSecret(masterKey, encryptedSecret)

	assert.Equal(t, actualDecryptedSecret, expectedDecryptedSecret)
	assert.True(t, len(actualDecryptedSecret) > 1)
}
